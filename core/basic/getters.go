...
func GetBalance(ethAddress [20]byte) (big.Int, error) {
	...
	response, err := utils.GetBalance(hexAddress)
	if err != nil {
...
func GetEarnings(ethAddress [20]byte) (big.Int, error) {
	...
	response, err := utils.GetStorageAt(hexStoragePosition)
	if err != nil {
		return *new(big.Int), err
	}
	var ret big.Int
...
type TxLog utils.TxLog
type TxLogs utils.TxLogs
type TxReceipt utils.TxReceipt

func GetTxLogs(txid string) (TxLogs, error) {
	receipt, err := utils.GetTxReceipt(txid)
	if err != nil {
		var empty TxLogs
		return empty, err
	}
...
// before they can upload to the ethfs cloud.
...
// called by routines in ethfs-dht since in the smart contract
